    ...
    def send_cartesian_trajectory(self, trajectory):
        ...
        for point in trajectory:
            pose_list.append(geometry_msgs.Pose(
                geometry_msgs.Vector3(point['point'][0], point['point'][1], point['point'][2]),
        ...
        for pose,time in zip(pose_list, time_list):
            ...
            trajectory_point.twist = geometry_msgs.Twist(linear = geometry_msgs.Vector3(0.005,0.005,0.005), angular = geometry_msgs.Vector3(0.005,0.005,0.005))
            trajectory_point.acceleration = geometry_msgs.Accel(linear = geometry_msgs.Vector3(0.005,0.005,0.005), angular = geometry_msgs.Vector3(0.005,0.005,0.005))
...
if __name__ == "__main__":
    ...
    path = '/home/when/github/crafts_extended/json/ur10e_milling_ROS_toolpath.json'
    with open(path,'r') as pat:
        toolpath = json.load(pat)

    config = toolpath[1]['data']['joint_values']

    # pose trajectory as a list of dictionary items with position and orientation
    client = TrajectoryClient()
    # client.send_joint_trajectory([config])
    client.send_cartesian_trajectory(trajectory = toolpath[2:])
